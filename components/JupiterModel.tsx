...
function JupiterModelInner({ url }: { url: string }) {
  ...
  const { rotationX, rotationY, rotationZ } = useSpring({
    rotationX: userInteracted ? (modelRef.current?.rotation.x || 0) : 0,
    rotationY: userInteracted ? (modelRef.current?.rotation.y || 0) : 0,
    rotationZ: userInteracted ? (modelRef.current?.rotation.z || 0) : 0,
  ...
  useFrame((_, delta) => {
    if (!userInteracted && modelRef.current) {
      // Rotación en Y (ajustada para Júpiter - más rápida que otros planetas)
    ...
    <div className="w-full h-full">
      ...
      >
        ...
        <Suspense fallback={<Loader />}>
          <JupiterModelInner url="/models/planets/jupiter.glb" />
        </Suspense>
